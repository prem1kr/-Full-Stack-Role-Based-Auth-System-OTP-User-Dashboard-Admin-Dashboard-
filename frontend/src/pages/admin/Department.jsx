...
const Department = () => {
    ...
    // Course Statistics
    const departmentStats = allProfiles.reduce((acc, student) => {
        const departments = student.branch;
        if (departments) { acc[departments] = (acc[departments] || 0) + 1; }
        return acc;
    }, {});

    // Filter Buttons
    const department = ["All", ...Object.keys(departmentStats)];
    ...
    const handleFilter = (branch) => {
        ...
        if (branch === "All") {
            ...
            const filtered = allProfiles.filter((student) => student.branch === branch);
